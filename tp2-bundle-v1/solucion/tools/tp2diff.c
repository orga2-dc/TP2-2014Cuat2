...
typedef struct bgr8_t {
	unsigned char b,g,r;
} __attribute__((packed)) bgr8_t;
...
int main(int argc, char **argv) {
	...
	if( (src_a = cvLoadImage (filename_a, CV_LOAD_IMAGE_UNCHANGED)) == 0 )
	{
	...
	if( (src_b = cvLoadImage (filename_b, CV_LOAD_IMAGE_UNCHANGED)) == 0 )
	{
		printf("Error al cargar: %s\n", filename_b);
	...
	for (int i = 0; i<height; i++) {
		for (int j = 0; j<width; j++) {
			bgr8_t *p_a = (bgr8_t*) &src_a_matrix[i][j*3];
			bgr8_t *p_b = (bgr8_t*) &src_b_matrix[i][j*3];


			if (abs(p_a->r - p_b->r) > epsilon ||
				abs(p_a->g - p_b->g) > epsilon ||
				abs(p_a->b - p_b->b) > epsilon)
			{
				if (!quiet) {
					printf("Hay diferencias en el pixel: (%4d, %4d) ", i, j);
					printf("[b %2x, g %2x, r %2x] vs. [b %2x, g %2x, r %2x]\n", 
							p_a->b, p_a->g, p_a->r, p_b->b, p_b->g, p_b->r);
				}
...
void imprimir_ayuda ( ) {
	...
	printf ( "                                                                  \n" );
	printf ( "    -q, --quiet       Ejecuta en modo silencioso.		            \n" );
}
